...
export function renderScaleUi (state) {
  ...
  //console.log('values', valuePercents, values)

  const subTools = <span className='scalingSubTools'>
    <div className='transformsGroup'>
      {transformInputs({fieldName: 'sca', extraKlasses:['absScaling'], unit: '', showPercents: true, step: transformStep, values, valuePercents, precision, min})}
    </div>
